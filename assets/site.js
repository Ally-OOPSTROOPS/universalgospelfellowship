...
(function() {
  ...
  // Formspree's free plan ignores the _next redirect field, so we submit the
  // opt-in in the background (AJAX) and then send the visitor to their gift
  // page ourselves. The hidden Formspree fields still deliver the lead to the
  // ministry inbox; the visitor always lands on the prayer.
  function initOptinForm() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        // Whether or not Formspree returns ok, send the visitor to their gift.
        // (Submission almost always succeeds; the priority is that the hurting
        // person receives the prayer.)
